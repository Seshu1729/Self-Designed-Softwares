...
def unlock_question(request):
    ...
    try:
        selected_question = quiz_game.objects.get(question_link=question_link, game_link=game_id, question_lock=-1)
...
def submit_answer(request):
    ...
    if comp(user_answer,question_obj.get_correct_options()):
        report_obj.questions_answered += 1
        report_obj.tame_taken += int(time_taken)
...
def leader_board(request):
    ...
    game_id = request.GET["game_id"]
    game_obj = game.objects.get(game_id=int(game_id))
    report_obj = report.objects.filter(game_id=game_obj).order_by('-questions_answered','tame_taken')

    zingzing(host_addr,'P',report_obj[0].table_id,2)
    zingzing(host_addr_sound,'1',report_obj[0].table_id,2)
