...
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "first_round.settings")
application = get_wsgi_application()

from fastest_finger_first.models import *
from django.contrib.auth.models import User

username = "seshu"
password = "seshu"
database_name = "first_round"
...
def upload_questions(db_name,sheet_name,row_count):
    ...
    for i in xrange(1, int(row_count) + 1):
        ...
        try:
            ...
            correct_options = [xl(ws,i,2),xl(ws,i,3),xl(ws,i,4),xl(ws,i,5)]
            display_options = list(correct_options)
            shuffle(display_options)

            #save new question
            question_obj = question(question_description=question_description)
            question_obj.set_display_options(display_options)
            question_obj.set_correct_options(correct_options)
            question_obj.save()
        except:
            pass
...
def upload_users():
    ...
    try:
        ws = openpyxl.load_workbook("Db/FFFDB.xlsx")["Sheet3"]
    except IOError:
